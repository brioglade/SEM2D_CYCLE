...
BcTopCz = zeros(ng,1);
for ex=1:NELX,
    ip = (NGLL-1)*(ex-1)+[1:NGLL-1] ;
    e = (NELY-1)*NELX+ex;
    BcTopIglob(ip) = iglob(1:NGLL-1,NGLL,e);
...
%    else
    ...
    impedance2 = RHO*VP2;
%    end
    BcTopCx(ip) = BcTopCx(ip) + dx_dxi*wgll(1:end-1)*impedance ;
    BcTopCz(ip) = BcTopCz(ip) + dx_dxi*wgll(1:end-1)*impedance2;
end
