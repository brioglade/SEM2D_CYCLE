...
dnew = zeros(length(FaultNIglob),1);	
Fnew = zeros(length(FaultNIglob),1);	
anew = zeros(length(FaultNIglob),1);	
rnew = zeros(length(FaultNIglob),1);	
pnew = zeros(length(FaultNIglob),1);	 
...
for e=1:NEL, % FOR EACH ELEMENT ...
    ...
    for k = 1:NGLL
        for j = 1:NGLL
            Klocdiag(k,j)=Klocdiag(k,j)+sum(coefint1*H(k,:)'.*(wloc(:,j).*Ht(:,k))...
...
v(:) = v(:) - 0.5*Vpl;
Vf = 2*v(FaultIglob);
iFBC = find(abs(FaultX)>=22.5*10^3/distN);
NFBC = length(iFBC);
Vf(iFBC) = 0;
...
for ex = 1:NELX
    for k = 1:NGLL
       if abs(x(iglob(k,1,ex))) >= 22.5*10^3/distN 
...
while t < tmax,
    ...
    % solve d = K^-1 F by PCG
    [dnew,n1(p1)]=myPCGnew2(coefint1,coefint2,diagKnew,dnew,F,FaultIglob,...
        FaultNIglob,H,Ht,iglob,NEL,nglob,W);
    ...
    Maeff = Mq(FaultIglob).*aeff;
    
    %% P_Ma based on max
    P_Ma(it)=max(max(abs(Maeff)./abs(a(FaultIglob))),max(abs(Maeff)./(abs(FaultB.*tau))));
    ...
    for jF=1:FaultNglob-NFBC
        ...
        [Vf1(j),tau1(j)]=NRsearch_NEW(fo(j),Vo(j),cca(j),ccb(j),Seff(j),...
            tauNR(j),tauo(j),psi1(j),FaultZ(j),FaultVFree(j));
    ...
    if(mod(it,everyN) == 0)
        ...
        getframe;
        figure(2);
        plot(x(FaultIglob),log10(v(FaultIglob)),c);
        getframe;
        hold on
        figure(3);
